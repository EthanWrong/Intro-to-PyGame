"""Snake - v16
Re-did the rendering system, made it look nicer

To add:
- Feature: when resuming game, snake jumps 2 blocks instead of one
...
class Screen:
    ...
    def show_screen(self, board, player, death_screen=False, start_screen=False, pause=False):
        ...
        lighter_shade_of_grass = True  # toggleable variable to create checkerboard background
        ...
        for r, row in enumerate(board.grid):
            ...
            for c, col in enumerate(row):
                ...
                lighter_shade_of_grass ^= True  # toggles variable
                colour = self.key["grass"][0] if lighter_shade_of_grass else self.key["grass"][1]  # creates checkerboard
                ...
                if col in ("body", "head"):  # snake head and body
                    ...
                    apple_img = pygame.image.load("Images/minecraft_apple.png").convert_alpha()
                    apple_sprite = pygame.transform.smoothscale(apple_img, [self.block_size - 1, self.block_size - 1])
